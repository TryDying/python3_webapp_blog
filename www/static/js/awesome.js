...
function parseQueryString() {
  ...
  if (q && q.charAt(0) === "?") {
    ...
    for (i = 0; i < qs.length; i++) {
      ...
      r[s.substring(0, pos)] = decodeURIComponent(s.substring(pos + 1)).replace(
        /\+/g,
        " "
      );
    }
...
$(function () {
  $(".x-smartdate").each(function () {
    $(this)
      .removeClass("x-smartdate")
      .text(toSmartDate($(this).attr("date")));
  });
});
  ...
  $.fn.extend({
    showFormError: function (err) {
      return this.each(function () {
        ...
        if (err) {
          $alert
            .text(err.message ? err.message : err.error ? err.error : err)
